...
セッションを跨いで「何をやろうとしていて、どこまで進んだか」を引き継ぐため、`docs/` を **input / output の 2 ディレクトリ**で運用する。

- `docs/input/<YYYY-MM-DD>_<topic>.md` — その取り組みの **plan / 設計判断 / 合意事項**。原則不変。設計判断が大きく変わったら末尾に「補遺」セクションを追加するか、新しい input ファイルを切る
- `docs/output/<YYYY-MM-DD>_<topic>.md` — 同名の input に対応する **進行ログ**。ステップを進めるごとに「完了したこと / 学習したこと / 次のステップ」を Claude Code が追記する
- 古いセッションログ (`docs/output/2026-06-22_*` / `2026-06-27_*`) は output 性質なので `docs/output/` 配下に集約済み

新しいセッションは:

1. **最新の `docs/input/` を読む** → 「何をやろうとしているか・どんな設計判断で進んでいるか」を把握
2. **対応する最新の `docs/output/` を読む** → 「どこまで進んだか・直近の議論」を把握
3. その続きから着手する
